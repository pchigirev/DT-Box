...
class SocketSrv:
    ...
    def accept_client(self) -> bool:
        try:
            ...
            #send_buf_size = self.serv.getsockopt(SOL_SOCKET, SO_SNDBUF)
            #print(f"Current send buffer size: {send_buf_size} bytes")
    
            #recv_buf_size = self.serv.getsockopt(SOL_SOCKET, SO_RCVBUF)
            #print(f"Current receive buffer size: {recv_buf_size} bytes")
